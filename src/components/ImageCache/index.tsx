...
export interface ICachedImageURISource extends ImageURISource {
  uri: string;
}

interface ICacheEntry {
  source: ICachedImageURISource;
  downloading: boolean;
...
export class ImageCache {
  private getPath(uri: string, immutable?: boolean): string {
    ...
    path = !path.includes('?') ? path : path.substring(path.lastIndexOf('.'), path.indexOf('?'));
    const ext = !path.includes('.') ? '.jpg' : path.substring(path.indexOf('.'));
    if (immutable === true) {
  ...
  private cache: {[uri: string]: ICacheEntry} = {};
  ...
  on(source: ICachedImageURISource, handler: CacheHandler, immutable?: boolean) {
    const {uri} = source;
    if (!this.cache[uri]) {
      this.cache[uri] = {
        source,
  ...
  private download(cache: ICacheEntry) {
    const {source} = cache;
    const {uri} = source;
    if (!cache.downloading) {
...
export interface ICachedImageProps extends ImageProperties {
  mutable?: boolean;
}

export interface CustomICachedImageProps extends ICachedImageProps {
...
export abstract class BaseCachedImage<P extends ICachedImageProps> extends Component<
  P,
  CachedImageState
> {
  private uri: string;
  ...
  private observe(source: ICachedImageURISource, mutable: boolean) {
  ...
  protected getProps() {
    ...
    Object.keys(this.props).forEach((prop) => {
      ...
      } else if (!['mutable', 'component'].includes(prop)) {
  ...
  componentWillMount() {
    ...
    if (typeof source !== 'number' && source.uri) {
      this.observe(source as ICachedImageURISource, mutable === true);
    }
  }
...
export class CachedImage extends BaseCachedImage<ICachedImageProps> {
  render() {
...
export class CachedImageBackground extends BaseCachedImage<ICachedImageProps> {
  render() {
    const props = this.getProps();
...
export class CustomCachedImage<P extends CustomICachedImageProps> extends BaseCachedImage<P> {
  render() {
